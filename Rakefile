...
YAJL_RUBY_VERSION = "1.2.1"  # Must match Gemfile
...
namespace :package do
  namespace :linux do
    ...
    task :x86 => [
      ...
      "packaging/traveling-ruby-#{TRAVELING_RUBY_VERSION}-linux-x86-yajl-ruby-#{YAJL_RUBY_VERSION}.tar.gz",
    ...
    task :x86_64 => [
      ...
      "packaging/traveling-ruby-#{TRAVELING_RUBY_VERSION}-linux-x86_64-yajl-ruby-#{YAJL_RUBY_VERSION}.tar.gz",
  ...
  task :osx => [
    ...
    "packaging/traveling-ruby-#{TRAVELING_RUBY_VERSION}-osx-yajl-ruby-#{YAJL_RUBY_VERSION}.tar.gz",
...
[
  ...
  file x86_file do
    if x86_file =~ /linux-x86\.tar\.gz\Z/
      download_runtime("linux-x86")
    else
      x86_file.match(/packaging\/traveling-ruby-#{TRAVELING_RUBY_VERSION}-linux-x86-(.*)\.tar\.gz\Z/)
      download_native_extension("linux-x86", $1)
    end
  end
end
...
[
  ...
  file x86_64_file do
    if x86_64_file =~ /linux-x86_64\.tar\.gz\Z/
      download_runtime("linux-x86_64")
    else
      x86_64_file.match(/packaging\/traveling-ruby-#{TRAVELING_RUBY_VERSION}-linux-x86_64-(.*)\.tar\.gz\Z/)
      download_native_extension("linux-x86_64", $1)
    end
  end
end
...
[
  ...
  file osx_file do
    if osx_file =~ /linux-osx\.tar\.gz\Z/
      download_runtime("osx")
    else
      osx_file.match(/packaging\/traveling-ruby-#{TRAVELING_RUBY_VERSION}-osx-(.*)\.tar\.gz\Z/)
      download_native_extension("osx", $1)
    end
  end
end
...
def create_package(target)
  ...
  sh "cp packaging/wrapper.sh #{package_dir}/main"
  sh "cp -pR packaging/vendor #{package_dir}/lib/"
  sh "cp Gemfile Gemfile.lock #{package_dir}/lib/vendor/"
  sh "mkdir #{package_dir}/lib/vendor/.bundle"
  sh "cp packaging/bundler-config #{package_dir}/lib/vendor/.bundle/config"
  sh "tar -xzf packaging/traveling-ruby-#{TRAVELING_RUBY_VERSION}-#{target}-yajl-ruby-#{YAJL_RUBY_VERSION}.tar.gz " +
  ...
  if !ENV['DIR_ONLY']
    sh "mkdir -p pkgs"
    sh "tar -czf pkgs/#{package_dir}.tar.gz #{package_dir}"
    sh "rm -rf #{package_dir}"
  end
end
